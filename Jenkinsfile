  ...
  stages {
    ...
    stage('Seed Test Database') {
      steps {
        ...
        sh 'docker-compose run --rm laravel-api php artisan migrate:fresh --seed'
      }
    }
    ...
    stage('Run Frontend Tests') {
      steps {
        dir("${UI_DIR}") {
          ...
          sh 'docker-compose run --rm angular-ui npm install --force'
    ...
    stage('Build Angular App') {
      steps {
        ...
        sh 'docker-compose run --rm angular-ui npm run build'
      }
    }

    stage('Deploy') {
      steps {
        echo '🚀 Starting containers for deploy environment...'
